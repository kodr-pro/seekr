...
fn create_mock_context(
    tm: TaskManager,
    registry: Option<seekr::tools::SkillRegistry>,
    ...
    seekr::tools::ExecutionContext {
        ...
        registry: std::sync::Arc::new(
            registry.unwrap_or_else(|| seekr::tools::SkillRegistry::new(None)),
        ),
        config,
        lsp_manager: std::sync::Arc::new(seekr::lsp::LspManager::new(std::path::PathBuf::from(
            "/tmp",
        ))),
...
async fn test_shell_command_simple() {
    ...
    assert!(
        result.to_lowercase().contains("hello world"),
        "Result was: {}",
        result
    );
...
async fn test_shell_command_interactive() {
    ...
    let handle = tokio::spawn(async move {
        shell_command(
            &args,
            &create_mock_context(tm_clone.clone(), None),
            Some(1),
            Some(1),
        )
        .await
    ...
    assert!(
        matches!(event1, AgentEvent::Activity(_)),
        "Expected Activity event, got {:?}",
        event1
    );

    // 2. Wait for the ShellInputNeeded event
    println!("Waiting for ShellInputNeeded event...");
    let (context, input_tx_from_event) =
        match tokio::time::timeout(std::time::Duration::from_secs(10), evt_rx.recv()).await {
    ...
    assert!(
        result.contains("RESULT:hello_from_test"),
        "Result was: {}",
        result
    );
}
...
async fn test_shell_command_stderr_prompt() {
    ...
    assert!(
        context.contains("[sudo] password") || context.is_empty(),
        "Context was: {}",
        context
    );
    ...
    assert!(
        result.contains("RESULT:secret_pass"),
        "Result was: {}",
        result
    );
}
...
async fn test_task_manager_activities() {
    ...
    tm.log_activity(
        "test_tool",
        "test summary",
        seekr::tools::task::ActivityStatus::Success,
        None,
        None,
    );
...
async fn test_execute_tool_mock() {
    ...
    let (result, activity) = execute_tool(
        "shell_command",
        &args,
        &create_mock_context(tm.clone(), Some(registry.clone())),
        Some(1),
        Some(1),
    )
    .await;
    ...
    if result.contains("Command completed with exit code:") {
        ...
        assert!(
            result.to_lowercase().contains("execute_tool test"),
            "Result was: {}",
            result
        );
    }
...
async fn test_parallel_file_reads() {
    ...
    let h1 = tokio::spawn(async move {
        execute_tool(
            "read_file",
            &args1,
            &create_mock_context(tm1.clone(), Some(reg1.clone())),
            Some(1),
            Some(2),
        )
        .await
    ...
    let h2 = tokio::spawn(async move {
        execute_tool(
            "read_file",
            &args2,
            &create_mock_context(tm2.clone(), Some(reg2.clone())),
            Some(2),
            Some(2),
        )
        .await
...
async fn test_streaming_content_preservation() {
    use seekr::agent::AgentEvent;
    use seekr::app::{App, AppMode, ChatEntry};

    let mut app = App::new_setup();
    app.mode = AppMode::Main;

    let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
    app.agent.event_rx = Some(rx);

    // 1. Assistant starts talking
    tx.send(AgentEvent::ContentDelta("Part 1".to_string()))
        .unwrap();
    ...
    tx.send(AgentEvent::ContentDelta("Part 2".to_string()))
        .unwrap();
    app.poll_agent_events();
    ...
    assert!(
        all_content.contains("Part 1"),
        "Part 1 missing. Entries: {:?}",
        app.chat_entries
    );
    assert!(
        all_content.contains("Part 2"),
