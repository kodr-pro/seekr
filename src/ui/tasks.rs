...
pub fn render_tasks(
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            Constraint::Min(5),
            Constraint::Length(if live_activities.is_empty() {
                0
            } else {
                (live_activities.len() as u16 * 2) + 2
            }),
...
fn render_active_threads(
    ...
    for (i, activity) in live.iter().enumerate() {
        let label = format!(
            " Thread {}: {} ",
            activity.thread_id.unwrap_or(0),
            activity.tool_name
        );
